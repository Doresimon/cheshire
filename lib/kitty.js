...
class Kitty {
  ...
  static async sendKitty(fromadd, toadd, kittyId){
    const kittyCore = Contract.declaration('KittyCore').at(process.env.ADDRESS_KITTY_CORE)
    console.log("3")
    console.log(toadd)
    console.log(fromadd)
    console.log(kittyId)
    await kittyCore.sendKitty.call(
      toadd,
      kittyId,
      { from: fromadd} // eslint-disable-line comma-dangle
    )
    await kittyCore.sendKitty(
      toadd,
      kittyId,
      { from: fromadd} // eslint-disable-line comma-dangle
    )
    ...
    console.log("5")
    const api_object = JSON.parse(kitty.api_object)
    api_object.owner.address = toadd
    console.log("6")
    return new Promise((resolve, reject) => {
      this.db.run('UPDATE kitties SET owner=?, api_object=? WHERE id_testnet=?', toadd, JSON.stringify(api_object), kittyId, (err) => {
        if (err) {
          reject(err)
        } else {
          resolve(kittyId)
